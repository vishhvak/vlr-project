...
def hierarchical_dataset(root, opt, select_data='/'):
    ...
    for dirpath, dirnames, filenames in os.walk(root+'/'):
        if not dirnames:
            ...
            augmented_dataset=RawDataset(root="/home/ubuntu/deep-text-recognition-benchmark/data_lmdb_release/training/output",labels_path="/home/ubuntu/deep-text-recognition-benchmark/data_lmdb_release/training/labels.txt",opt=opt)
            dataset_list.append(augmented_dataset)
...
class RawDataset(Dataset):
    ...
    def __init__(self, root, labels_path,opt):
        self.opt = opt
        ...
        with open(labels_path,"r") as f:
            self.labels_list=f.readlines() 

        self.labels_list=[x.split(",")[1].strip() for x in self.labels_list]
